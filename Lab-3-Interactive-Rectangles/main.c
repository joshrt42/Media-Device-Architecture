...
void update() {
	...
	if (BUTTON_HELD(BUTTON_UP) && bRow > 0) {
		bRow -= bRDel;
	}
	if (BUTTON_HELD(BUTTON_DOWN) && bRow < SCREENHEIGHT - bHeight) {
		bRow += bRDel;
	}
	if (BUTTON_HELD(BUTTON_LEFT) && bCol > 0) {
		bCol -= bCDel;
	}
	if (BUTTON_HELD(BUTTON_RIGHT) && bCol < SCREENWIDTH - bWidth) {
		bCol += bCDel;
	}
