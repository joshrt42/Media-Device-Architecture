...
void setPixel(int row, int col, unsigned char colorIndex) {
 ...
 unsigned short offset = ((row)*(240)+(col))/2;
...
void drawImage(int row, int col, int height, int width, const unsigned short *image) {
    ...
    for (int r = 0; r < height; r++){
        DMANow(3, image + (r*width/2), &videoBuffer[((row + r)*(240)+(col))/2], width/2);
    }
}
...
void refreshGame() {
 ...
 DMANow(3, refresherBitmap, &videoBuffer[((24)*(240)+(count))/2], 240 * 108/2);
