...
# 1 "player.h" 1
# 20 "player.h"
extern const unsigned short playerBitmap[144];
# 18 "game.c" 2
# 1 "enemy.h" 1
# 20 "enemy.h"
extern const unsigned short enemyBitmap[432];
# 19 "game.c" 2
...
void initPlayer() {
    player.row = (160/2)-(playerHeight/2);
    player.col = 20;
...
void updatePlayer() {
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<6))) && player.row > screenbound + 2) {
        player.row -= player.rdel;
    } else if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<7))) && player.row < 160 - screenbound - player.height - 4) {
        player.row += player.rdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5))) && player.col > 2) {
        player.col -= player.cdel;
    } else if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && player.col < 240 - player.width - 10) {
        player.col += player.cdel;
    }


    if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0)))) && player.bulletTimer >= 10) {
        fireBullet();
    }
...
void updateEnemy(ENEMY* e) {
    ...
    if (e->active) {
        ...
        if ((e->row < 26) || (e->row > (160 - 29 - e->height)))
            e->rdel *= -1;
        e->row += e->rdel;
