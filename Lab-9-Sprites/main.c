...
// whenever pikachu is idle, just show whatever state pikachu was before
...
int main() {
	...
	while(1) {
		...
		// Look for the TODO past the button input section for more explanation
        ...
        //           Else, if pikachu aniState is not idle, we want pikachuAniCounter++
        if (pikachu.aniState == PIKAIDLE) {
            pikachu.curFrame = 0;
        ...
        //           and the previous animation state should be the one you show.
        shadowOAM[0].attr0 = pikachu.row | ATTR0_SQUARE;
        shadowOAM[0].attr1 = pikachu.col | ATTR1_SMALL;
        shadowOAM[0].attr2 = ATTR2_TILEID(pikachu.curFrame*2, pikachu.aniState*2);
...
void initialize() {
    ...
    loadPalette(bgPal);
    REG_BG0CNT = BG_SIZE_LARGE | BG_CHARBLOCK(0) | BG_4BPP | BG_SCREENBLOCK(28);
    ...
    // TODO 3.1: Hide all sprites with hideSprites().  You must complete this function yourself in myLib.c.
    hideSprites();
